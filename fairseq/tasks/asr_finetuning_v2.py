...
@dataclass
class ASRFinetuningConfig(FairseqDataclass):
...
@register_task("asr_finetuning_v2", dataclass=ASRFinetuningConfig)
class ASRFinetunig_v2(FairseqTask):
    def __init__(
        self,
    ...
    def _inference_with_wer(self, generator, sample, model):
        ...
        def decode(toks):
            s = self.bart.decode(toks.int().cpu())
            return s
    ...
    #         assert tokens.dim() == 1
    #         tokens = tokens.cpu().int().numpy()
    #         if tokens[0] == self.target_dictionary.bos():
    #             tokens = tokens[1:]  # remove <s>
    #         eos_mask = tokens == self.target_dictionary.eos()
    #         doc_mask = eos_mask[1:] & eos_mask[:-1]
    #         sentences = np.split(tokens, doc_mask.nonzero()[0] + 1)
    #         sentences = [
    #             self.bpe.decode(self.target_dictionary.string(s)) for s in sentences
    #         ]
    #         if len(sentences) == 1:
    #             return sentences[0]
    #         return sentences[0]
    ...
    #         ## TODO cfg加入debug选项，可以调试输出decode结果
    #         if self.cfg.debug:
    #             print("predict:", hyp)
    #             print("target: ", ref)
    #         num_char_errors += editdistance.eval(hyp, ref)
